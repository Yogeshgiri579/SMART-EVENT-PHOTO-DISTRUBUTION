      ...
      try {
        ...
        if (err.response?.status === 400 && err.response?.data?.message === 'Email already registered') {
          try {
            ...
            setError('Account exists, but password incorrect. Please try again.')
            setIsAuthLoading(false)
            return
          }
        } else {
          setError(err.response?.data?.message || err.response?.data?.error || 'Account creation failed')
          setIsAuthLoading(false)
          return
        }
      }
            ...
            {step === 1 && (
              <>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  >
                    {mutation.isPending ? 'Registering...' : (user ? 'Register for Event' : 'Next Step →')}
                  </button>
                  <button
                    type="button"
            ...
            {step === 2 && !user && (
              <div className="space-y-4">
                <div className="bg-primary-50 border border-primary-200 rounded-lg p-4 mb-4">
                  <p className="text-primary-800 text-sm">You're almost there! We just need your email to create your account or sign you in securely.</p>
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    type="submit"
                    disabled={isAuthLoading || mutation.isPending}
                    className="flex-1 px-6 py-3 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
                  >
                    {(isAuthLoading || mutation.isPending) && (
